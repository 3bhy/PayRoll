...
public class FhirClientService {
	...
	public MethodOutcome sendSimplePatient() {
		...
		patient.addName().setFamily("Ahmed").addGiven("Ali");
	...
	public void readPatient(IIdType patientId) {
		...
		Patient patient = client.read().resource(Patient.class).withId(logicalId).execute();
		...
		System.out.println("No Patient to read");
	...
	public Patient updatePatient(IIdType patientId) {
		...
		Patient existing = client.read().resource(Patient.class).withId(logicalId).execute();

		existing.getNameFirstRep().setFamily("UpdatedFamily");

		existing.getNameFirstRep().getGiven().clear();
		existing.getNameFirstRep().addGiven("UpdatedGiven");

		MethodOutcome updatedOutcome = client.update().resource(existing).withId(logicalId).execute();

		Patient updated = (Patient) updatedOutcome.getResource();
		System.out.println("Updated Patient: " + updated.getId());
		System.out.println("Updated Patient: " + updated.getNameFirstRep().getFamily());
		System.out.println("Updated Patient: " + updated.getNameFirstRep().getGivenAsSingleString());
	...
	public void deletePatient(String id) {
		try {
		IIdType pid = new IdType("Patient", id);

		client.delete().resourceById(pid).execute();
